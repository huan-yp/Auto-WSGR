...
# ========== 游戏设置 ==========
emulator:
  type: "雷电" # 模拟器类型，现在只支持雷电和蓝叠 Hyper-V两种
  start_cmd: "C:/leidian/LDPlayer9/dnplayer.exe" #必须填写，不然会报错
...
daily_automation:
  ...
  battle_type: "困难驱逐" # 打哪个战役。可选名称参考"plans/battle/"
...
日常使用脚本需要在盘符 例如：G\Auto-WSGR-main\examples文件夹里面配置user_settings.yaml文件。
活动脚本有专用的脚本启动在 盘符\Auto-WSGR-main\examples event文件同样需要配置。
以上两个配置文件均可以在 盘符\Auto-WSGR-main\autowsgr\data\plans文件夹里查看。活动文件为event文件夹内对应最近日期的文件。
...
**为了方便决战, 该文件中 "鲃鱼" 已经改名成 "肥鱼", 如果你有其它特殊需要, 可以手动改回.**
...
另外，由于 `easyocr` 工具本身可能没有收录一些中文字体, 所以无法识别部分生僻字, 经典例子是日系动物园和 "鲃鱼", 这里推荐把 "鲃鱼" 改名为 "肥鱼" (只需要在游戏中更改), 以后会解决这个问题.
