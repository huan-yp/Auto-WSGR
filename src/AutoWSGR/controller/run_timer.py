...
class Timer(Emulator):
    ...
    def is_bad_network(self, timeout=10):
        return (
            self.wait_images(
                [IMG.error_image["bad_network"], IMG.symbol_image[10]], timeout=timeout
            )
            != None
        )
    ...
    def identify_page(self, name, need_screen_shot=True):
        ...
        return any(
            self.image_exist(template, 0) for template in IMG.identify_images[name]
        )
    ...
    def ConfirmOperation(self, must_confirm=0, delay=0.5, confidence=0.9, timeout=0):
        ...
        res = self.get_image_position(
            IMG.confirm_image[pos + 1], confidence=confidence, need_screen_shot=0
        )
...
def process_error(timer: Timer):
    ...
    if (
        timer.Windows.is_android_online() == False
        or timer.Android.is_game_running() == False
    ):
