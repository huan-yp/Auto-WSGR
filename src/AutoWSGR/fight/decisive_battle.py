...
class DecisiveBattle:
    ...
    def choose(self, refreshed=False, rec_only=False):
        ...
        self.stats.score = int(
            recognize_number(
                screen[25:55, 1162:1245], min_size=5, text_threshold=0.05, low_text=0.02
            )[0][1]
        )
        ...
        for i, cost in enumerate(costs):
            ...
            ships.append(
                _recognize_ship(
                    screen[488:515, DSP[i][0] : DSP[i][1]], self.timer.ship_names
                )[0][0]
            )
        ...
        costs = _costs
        selections = {
            ships[i]: (costs[i], CHOOSE_POSITION[real_position[i]])
    ...
    def use_skill(self, type=3):
        ...
        if type == 3:
            ships = _recognize_ship(
                self.timer.get_screen()[488:515], self.timer.ship_names
            )
            for ship in ships:
    ...
    def _get_chapter(self):
        return int(
            recognize_number(self.timer.get_screen()[588:618, 1046:1110], "Ex-X")[0][1][
                -1
            ]
        )
