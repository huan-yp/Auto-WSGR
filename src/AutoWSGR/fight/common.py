...
class FightEvent:
    ...
    def __str__(self) -> str:
        return (
            f"事件:{self.event}, 状态:{self.stats}, 动作:{self.action}, 结果:{str(self.result)}"
        )
...
class FightInfo(ABC):
    ...
    def update_state(self):
        ...
        while time.time() - fun_start_time <= timeout:
            ...
            # 尝试匹配
            ret = [
                self.timer.images_exist(image, False, confidence=confidence)
                for image in images
            ]
            if any(ret):
...
class FightPlan(ABC):
    ...
    def update_state(self):
        try:
            ...
            if self.Info.last_state in ["proceed", "night"] and self.timer.images_exist(
                IMG.fight_image[5:7]
            ):
